...
use crate::io::s3_objects::aws_s3_objects_io::AwsS3ObjectsIO;
...
pub async fn init_s3_objects_io(config: &GalleryIoConfig) -> Result<Arc<dyn S3ObjectsIO>> {
    let io: Arc<dyn S3ObjectsIO> = match config.s3_objects {
        GalleryS3ObjectsIoImpl::Aws => Arc::new(AwsS3ObjectsIO::new(&config.s3).await?),
